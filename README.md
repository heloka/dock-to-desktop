...
窗口停靠后，可以拖动靠近桌面中央的内侧边框调整宽度。若按快捷键前的前台窗口处于同一屏幕并且已经最大化，插件会把它调整到剩余工作区；拖动速记边框时，两边会使用同一条分隔线同步改变宽度。收起速记后，原窗口恢复最大化，松开后会保存新的速记宽度。
...
如果原生组件不能加载或 Windows 拒绝注册，插件会先回滚可能存在的注册，再使用不挤占工作区的置顶窗口，并在 Obsidian 中显示原因。相邻窗口联动只处理呼出前位于目标屏幕、处于最大化状态的前台窗口；普通大小的窗口和其他屏幕上的窗口不会被移动。
