...
窗口停靠后，可以拖动靠近桌面中央的内侧边框调整宽度。若按快捷键前的前台窗口位于同一屏幕，且已经最大化或贴边占据大部分屏幕高度，插件会把它调整到剩余区域；拖动速记边框时，两边会使用同一条分隔线同步改变宽度。收起速记后，最大化窗口恢复最大化，贴边窗口恢复原有位置和大小。松开后会保存新的速记宽度。
...
如果 Windows 拒绝 AppBar 注册，插件会先回滚可能存在的注册，再使用置顶窗口；若相邻窗口接口仍可用，呼出前的前台窗口仍会随速记边框调整。若原生组件完全不可用，插件只显示置顶速记窗口。相邻窗口联动仅处理同屏、最大化或贴边占据大部分屏幕高度的前台窗口；小型窗口和其他屏幕上的窗口不会被移动。对已加入 Windows 贴靠组的窗口，收起时恢复其位置和大小，但 Windows 可能不会恢复原贴靠组关系。
